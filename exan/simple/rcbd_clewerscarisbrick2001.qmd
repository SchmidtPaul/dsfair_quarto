...
We make use of `dlookr::describe()`[^1] to conveniently obtain descriptive summary tables. Here, we get can summarize per block and per cultivar.

[^1]: As described in the {dlookr} section of the Summary Article "Useful things".
...
Finally, since this is an experiment that was laid with a certain experimental design (= a randomized complete block design; RCBD) - it makes sense to also get a field plan. This can be done via `desplot()` from [{desplot}](https://kwstat.github.io/desplot/). We can even create a second field plan that gives us a feeling for the yields per plot.
...
Thus, `C4` seems to be the most promising cultivar in terms of yield. Moreover, it can be seen that yields were generally higher in block `B1` (left), compared to the other blocks. \# Model
