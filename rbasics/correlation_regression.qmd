...
If you would like additional information, such as a confidence interval and a test resulting in a p-value, you can use `cor.test()` instead of `cor()`. We may also use the [{broom}](https://broom.tidymodels.org/) package to get the results in a more convenient format.
...
After fitting a model, you may use it to make predictions. Here is one way of obtaining the expected blood alcohol content for having 0 to 9 drinks according to our simple linear regression via [{modelbased}](https://easystats.github.io/modelbased/):
...
No, they are not. Yes, the sample estimate for the intercept is 0.049, but when looking at more detailed information via e.g. `summary()`. We may also use the [{broom}](https://broom.tidymodels.org/) package to get the results in a more convenient format.
